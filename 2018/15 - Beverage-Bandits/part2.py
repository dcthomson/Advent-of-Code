...
class Unit:
    ...
    def __str__(self):
        ...
        retstr += " hp-" + str(self.hp) + "  ap=" + str(self.ap)
        return retstr
    ...
    def runRound(self):
        for unit in self.getUnits():
            if unit.alive:
                ...
                if lowesthp:
                    if not unit.attack(lowesthp, self):
                        return "Elf died"
        self.round += 1
...
hcap = 0
whowon = False
boardstart = Board(file)
while True:
    board = deepcopy(boardstart)
    ...
    keepgoing = True
    while keepgoing:
        keepgoing = board.runRound()
        print(board)
        if keepgoing and keepgoing == "Elf died":
            break
    if keepgoing and keepgoing == "Elf died":
        hcap += 1
        print("\n")
